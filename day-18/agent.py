...
def initial_history(_server: Server) -> list[dict]:
    return [{"role": "system", "content": (
        "Ты помощник по журналу успешных SSH-аутентификаций на учебной VM. "
        "Для фактических ответов используй MCP-инструменты. Время в результатах Europe/Kirov (UTC+03:00). "
        "Если выдача ограничена или последний сбор устарел, явно сообщи об этом. "
        "Не делай выводов о неудачных попытках или действиях внутри сеанса."
    )}]
...
async def run_turn(
    ...
    key = api_key()
    if not key:
        raise RuntimeError("Добавьте DEEPSEEK_API_KEY в day-18/.env")
            ...
            for round_number in range(1, MAX_ROUNDS + 1):
                ...
                for call in calls:
                    ...
                    if len(serialized) > MAX_MODEL_RESULT_CHARS:
                        trace("RESULT LIMIT", f"Модели переданы первые {MAX_MODEL_RESULT_CHARS} символов результата")

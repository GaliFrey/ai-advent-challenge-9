"""Download and verify reports from the configured учебная VM."""
...
REMOTE_DIR = PurePosixPath('/home/heimdall/ai-advent-day19/reports')


async def read_remote(path: str) -> bytes:
    process = await asyncio.create_subprocess_exec(
        'ssh', '-T', '-F', str(Path.home() / '.ssh/config'), '-o', 'BatchMode=yes',
        '-o', 'ConnectTimeout=10', 'yc', 'cat', '--', path,
...
async def download_report(saved: dict) -> dict:
    ...
    if remote.parent != REMOTE_DIR or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9_-]{0,79}\.md', remote.name):

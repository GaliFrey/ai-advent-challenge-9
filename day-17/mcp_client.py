"""Connections to the local Datex and public documentation MCP servers."""
...
async def connect(server: Server) -> AsyncIterator[Client]:
    target = (
        StdioServerParameters(command=sys.executable, args=[str(ROOT / "server.py")])
        if server.local else server.address
    )

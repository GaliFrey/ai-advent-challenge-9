"""Изолированный агент с постоянной историей и статистикой запуска."""
...
class AgentConfig:
    ...
    def check(self) -> None:
        if not self.api_key.strip():
            raise ValueError("Добавь DEEPSEEK_API_KEY в day-07/.env")
...
def load_config() -> AgentConfig:
    ...
    try:
        ...
        raise ValueError("Проверь DEEPSEEK_MAX_TOKENS и DEEPSEEK_TEMPERATURE в day-07/.env") from error
    return AgentConfig(
...
class Agent:
    """Один агент со своей постоянной историей."""

    def __init__(self, config: AgentConfig, client: httpx.AsyncClient, store: JsonHistoryStore):
        config.check()
        persisted_messages = store.load()
        self.config = config
        self._client = client
        self._store = store
        ...
        self._stats = AgentStats(history_messages=len(persisted_messages))
        self._lock = asyncio.Lock()

    @property
    def messages(self) -> tuple[dict[str, str], ...]:
        """Защитная копия полной истории для диагностики и тестов."""
    ...
    async def ask(self, user_message: str) -> AgentReply:
        ...
        async with self._lock:
            ...
            if finish_reason != "stop":
                message = f"Ответ не завершён (finish_reason: {finish_reason}); история не изменена"
                self._record_error(message, started)
            ...
            try:
                self._store.save(committed_messages[1:])
            except HistoryStorageError as error:
                message = "Ответ получен, но историю сохранить не удалось; она не изменена"
                self._record_error(message, started)
                raise AgentError(message) from error

            self._messages = committed_messages
            self._stats = replace(
                self._stats,
    ...
    def clear_history(self) -> None:
        try:
            ...
            raise AgentError("Не удалось очистить сохранённую историю") from error
